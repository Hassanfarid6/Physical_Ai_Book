        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"Server error {response.status_code} on attempt {attempt + 1} for {url}. Retrying...")
                    time.sleep(self.delay * (2 ** attempt))
                else:
                    # Client error - not worth retrying in most cases
                    logger.error(f"Client error {response.status_code} on attempt {attempt + 1} for {url}. Skipping...")
                ...
                if attempt < self.max_retries:
                    ...
                    logger.error(f"Failed to fetch {url} after {self.max_retries} retries due to timeout")
            except requests.exceptions.ConnectionError:
                logger.warning(f"Connection error on attempt {attempt + 1} for {url}. Retrying...")
                if attempt < self.max_retries:
                    time.sleep(self.delay * (2 ** attempt))  # Exponential backoff
                else:
                    logger.error(f"Failed to fetch {url} after {self.max_retries} retries due to connection error")
