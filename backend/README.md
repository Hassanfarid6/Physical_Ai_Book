# Docusaurus Website Ingestion Pipeline

This project implements a pipeline that crawls Docusaurus websites, extracts content, generates Cohere embeddings, and stores them in Qdrant vector database.

## Overview

The ingestion pipeline performs the following steps:
1. Crawls a specified Docusaurus website to discover all public URLs
2. Extracts content from each page while preserving structural information
3. Chunks the content using a semantic approach that preserves meaning
4. Generates embeddings using Cohere models
5. Stores the embeddings with metadata (URL, section, chunk id) in Qdrant
...
The pipeline is implemented in a single file (`main.py`) with the following modules:
- `config.py`: Handles environment variables and configuration
- `url_discovery.py`: Discovers URLs from Docusaurus sites
- `content_extraction.py`: Extracts content from web pages
- `chunking.py`: Implements semantic chunking of content
- `models.py`: Defines data models (ContentChunk, EmbeddingVector, Metadata)
- `cohere_utils.py`: Handles Cohere API interactions
- `qdrant_utils.py`: Handles Qdrant database operations
- `logging_utils.py`: Provides logging and progress tracking
- `error_handling.py`: Implements error handling and retry logic
- `crawling_utils.py`: Implements respectful crawling behavior
- `temp_storage.py`: Provides temporary storage functionality
- `content_types.py`: Handles different content types (code blocks, tables)
